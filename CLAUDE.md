...
**Default currency and the migration.** The default currency lives on `users/{uid}` in Firestore — the app's only per-user document — because local storage is cleared on sign-out, and a device-only copy made returning users read stored BRL amounts as USD. kazi_core owns the contract (`KaziRemoteCurrencyStore`, overridden in `main.dart`); local storage is just a first-frame cache. Reads go through `kaziDefaultCurrencyProvider`. Two things keep the cache from becoming the source of truth: `selectCurrency` writes the store **first** and the cache only once that lands, rethrowing so the picker can say the choice was not saved; and `appRemoteCurrencyStoreProvider` watches `kaziIsAuthenticatedProvider`, because `KaziCurrencyController` is kept alive and first built on the splash, before anyone is signed in.

Every account confirms its currency once, inside the onboarding setup (see [onboarding/README.md](packages/kazi/lib/features/onboarding/README.md)) — there is no separate migration screen. The setup calls `CurrencyMigrationController.confirm`, which writes the currency, backfills legacy documents, and only **then** sets `currencyMigratedAt`; the setup's completion stamp is written after that, so an interrupted run reappears next launch and skips what it already stamped. The backfill pages with `where(FieldPath.documentId, isGreaterThan: lastId)`, not `startAfterDocument`, because `fake_cloud_firestore` returns nothing for cursor paging over `__name__`.
