...
class ServiceTypesCubit extends Cubit<ServiceTypesState> with BaseCubit {
  ...
  Future<void> deleteServiceType(ServiceType serviceType) async {
    try {
      ...
      emit(
        state.copyWith(status: BaseStateStatus.success, serviceTypes: newList),
      );
    } on AppError catch (exception) {
      onAppError(exception);
    } catch (exception) {
  ...
  void eraseServiceType() {
    emit(
      state.copyWith(serviceType: ServiceType(userId: _authService.user!.uid)),
    );
  }
  ...
  void changeServiceTypeDefaultValue(double value) => emit(
    state.copyWith(
      serviceType: state.serviceType.copyWith(defaultValue: value),
    ),
  );

  void changeServiceTypeDiscountPercent(double value) => emit(
    state.copyWith(
