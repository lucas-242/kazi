...
Kazi is a Melos-managed Flutter monorepo for an earnings/service tracking ecosystem aimed at freelancers and service providers. Three Dart packages under `packages/` are managed by Melos:
...
Two more folders under `packages/` are **not** Dart packages and are not in the Melos package list (so `analyze`/`test` skip them):

- **kazi_landing** — the static marketing site (pt/en/es) plus the web privacy policy, on Firebase Hosting (`staging` → kazi-clients-staging.web.app, `prod` → kazipro.io). Every `index.html` is generated by `tool/build.dart` from `tool/*.html` templates and `l10n/*.json` dictionaries — never edit the HTML by hand. The privacy-policy text is read from the kazi_core ARBs, so changing the policy means changing the ARB and regenerating; the app opens `/privacy-policy` (`AppUrls.privacyPolicy`), so that path must not move. The app screens on the page are WebP exports of kazi_mockups in `assets/screens/<lang>/`, not HTML. Details in [kazi_landing/README.md](packages/kazi_landing/README.md).
- **kazi_mockups** — app-screen mockups for the store listings and the landing page: six screens × three locales (`pt-BR`, `es-PY`, `en-US`), 1080×2160 PNGs in `<locale>/`. They are rendered from HTML by a Python + Playwright script in `generator/` (`data.py` holds the localized sample data, `render.py` the layout); all numbers derive from one seeded dataset so they add up across screens. `generator/landing.py` exports the screens the landing uses as WebP into `kazi_landing/assets/screens/`. Details in [kazi_mockups/README.md](packages/kazi_mockups/README.md).
